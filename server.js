...
function recommendedClientId(profile) {
  ...
  if (profile.family === "nxt") return "nxt-python-client";
  if (profile.family === "rcx") return "rcx-nqc-client";
  if (profile.family === "robot-inventor" || profile.family === "spike-prime") return "robot-inventor-51515";
  if (profile.family === "wedo2") return "wedo2-app";
  return "local-mcp";
}
...
function createOfficialClientHandoff(params = {}) {
  ...
  } else if (clientId === "nxt-python-client") {
    steps = [
      "Install nxt-python on your computer: pip install nxt-python",
      `Connect the NXT brick (${profileName}) via USB cable, or pair via Bluetooth.`,
      "Run the generated Python script: python3 program.py",
      "Watch which motors move and in which direction.",
      "Report exact movements, sounds, and any Python errors to the builder session."
    ];
  } else if (clientId === "rcx-nqc-client") {
    steps = [
      "Install NQC (Not Quite C) from bricxcc.sourceforge.net or use BricxCC IDE.",
      "Connect the LEGO IR tower to your computer (USB or legacy serial).",
      `Compile the generated NQC program: nqcc ${safeSlug(profileName)}.nqc`,
      `Download to the RCX brick: nqc -download program.rcx`,
      "Run Task 1 on the RCX and report exactly what moved to the builder session."
    ];
  } else if (clientId === "wedo2-app") {
    steps = [
      "Open the LEGO Education WeDo 2.0 App (available until July 31 2026).",
      `Connect the hub named 'LPF2 Smart Hub 2 I/O' via Bluetooth.`,
      `Select or create a project for ${profileName}.`,
      "Paste or recreate the generated MicroPython code in the app's Python editor.",
      "Run the program once with the robot on a clear surface.",
      "Report exact movement, hub LED colour, and any errors to the builder session.",
      "MIGRATION NOTE: When the WeDo 2.0 app is discontinued (July 31 2026), flash Pybricks firmware " +
      "(https://code.pybricks.com) and switch to the 'Pybricks CityHub' code target."
    ];
  } else {
    steps = [
...
function generatePybricksEv3Code(profile, intent, customCode) {
  ...
  const lines = [
    ...
    `# Safety constants — adjust carefully`,
    `SAFE_SPEED = 200   # deg/s`,
    `SAFE_ANGLE = 360   # degrees per motor sweep`,
    `WAIT_MS    = 500`,
    ``,
    `ev3 = EV3Brick()`,
    ``
  ...
  if (sensorEntries.length > 0) {
    ...
    for (const [port, p] of sensorEntries) {
      const varName = toVarName(p.role);
      const sPort = `Port.S${port}`;
      if (p.part === "color-sensor")       lines.push(`${varName} = ColorSensor(${sPort})  # ${p.role}`);
      else if (p.part === "ultrasonic-sensor") lines.push(`${varName} = UltrasonicSensor(${sPort})  # ${p.role}`);
      else if (p.part === "touch-sensor")  lines.push(`${varName} = TouchSensor(${sPort})  # ${p.role}`);
      else if (p.part === "infrared-sensor") lines.push(`${varName} = InfraredSensor(${sPort})  # ${p.role}`);
      else if (p.part === "gyro-sensor")   lines.push(`${varName} = GyroSensor(${sPort})  # ${p.role}`);
    }
    lines.push("");
  }
  lines.push("# Program");
  if (intent === "beep_hello") {
    lines.push(`ev3.speaker.beep(frequency=500, duration=500)`);
    lines.push(`ev3.light.on(Color.GREEN)`);
    lines.push(`wait(1000)`);
    lines.push(`ev3.light.off()`);
  } else if (intent === "safe_probe") {
    lines.push(`ev3.light.on(Color.YELLOW)`);
    for (const [port, p] of motorEntries) {
      const varName = toVarName(p.role);
      lines.push(`print("Testing ${p.role} (Port ${port})")`);
      lines.push(`${varName}.run_angle(SAFE_SPEED, SAFE_ANGLE)`);
    ...
    lines.push(`ev3.light.on(Color.GREEN)`);
    lines.push(`ev3.speaker.beep(frequency=800, duration=200)`);
    lines.push(`print("Probe complete")`);
  } else if (intent === "drive_forward") {
    ...
    if (arms.length > 0) {
      ...
      lines.push(`ev3.speaker.beep()  # No arm motor found for wave`);
    }
  } else if (intent === "custom" && customCode) {
    lines.push(String(customCode));
  } else {
    lines.push(`ev3.speaker.beep()`);
    lines.push(`wait(500)`);
  }
  return lines.join("\n") + "\n";
}
...
function generateEv3devCode(profile, intent, customCode) {
  ...
  if (motorEntries.length > 0) {
    ...
    for (const [port, p] of motorEntries) {
      ...
      lines.push(`${varName} = ${cls}(OUTPUT_${port})  # ${p.role}`);
    }
    lines.push("");
  }
  if (sensorEntries.length > 0) {
    ...
    for (const [port, p] of sensorEntries) {
      const varName = toVarName(p.role);
      if (p.part === "color-sensor")       lines.push(`${varName} = ColorSensor(INPUT_${port})  # ${p.role}`);
      else if (p.part === "ultrasonic-sensor") lines.push(`${varName} = UltrasonicSensor(INPUT_${port})  # ${p.role}`);
      else if (p.part === "touch-sensor")  lines.push(`${varName} = TouchSensor(INPUT_${port})  # ${p.role}`);
      else if (p.part === "infrared-sensor") lines.push(`${varName} = InfraredSensor(INPUT_${port})  # ${p.role}`);
      else if (p.part === "gyro-sensor")   lines.push(`${varName} = GyroSensor(INPUT_${port})  # ${p.role}`);
    }
    lines.push("");
  }
  lines.push("# Program");
  if (intent === "beep_hello") {
    lines.push(`sound.beep()`);
    lines.push(`leds.set_color("LEFT", "GREEN")`);
    lines.push(`leds.set_color("RIGHT", "GREEN")`);
    lines.push(`sleep(1)`);
  } else if (intent === "safe_probe") {
    for (const [port, p] of motorEntries) {
      const varName = toVarName(p.role);
      lines.push(`print("Testing ${p.role} (OUTPUT_${port})")`);
      lines.push(`${varName}.on_for_seconds(SAFE_SPEED, 1)`);
      lines.push(`sleep(WAIT_SECS)`);
      lines.push(`${varName}.on_for_seconds(-SAFE_SPEED, 1)`);
      lines.push(`sleep(WAIT_SECS)`);
    }
    lines.push(`sound.beep()`);
    ...
    if (drives.length >= 2) {
      lines.push(`# Drive forward 1 second`);
      for (const [, p] of drives) lines.push(`${toVarName(p.role)}.on(SAFE_SPEED)`);
      lines.push(`sleep(1)`);
      for (const [, p] of drives) lines.push(`${toVarName(p.role)}.off()`);
    } else if (drives.length === 1) {
      lines.push(`${toVarName(drives[0][1].role)}.on_for_seconds(SAFE_SPEED, 1)`);
    } else {
      lines.push(`# No drive motors found — add motor code for ${profile.name}`);
    }
  } else if (intent === "wave") {
    ...
    if (arms.length > 0) {
      ...
      const varName = toVarName(p.role);
      lines.push(`for _ in range(3):`);
      lines.push(`    ${varName}.on_for_degrees(SAFE_SPEED, 90)`);
      lines.push(`    sleep(0.2)`);
      lines.push(`    ${varName}.on_for_degrees(-SAFE_SPEED, 90)`);
      lines.push(`    sleep(0.2)`);
    } else {
      lines.push(`sound.beep()  # No arm motor found for wave`);
    }
  } else if (intent === "custom" && customCode) {
    lines.push(String(customCode));
  } else {
...
function generateNxtPythonCode(profile, intent, customCode) {
  ...
  for (const [, p] of sensorEntries) {
    ...
    else if (p.part === "sound-sensor") sensorImports.add("Sound");
  }
  const sensorLine = sensorImports.size
    ? `from nxt.sensor import ${[...sensorImports].join(", ")}` : "";
  const lines = [
    ...
    `import time`,
    ``,
    `# Connect to NXT brick (USB first, then Bluetooth)`,
  ...
  if (motorEntries.length > 0) {
    lines.push("# Motor bindings from profile");
    for (const [port, p] of motorEntries) {
      lines.push(`${toVarName(p.role)} = Motor(brick, MotorPort.${port})  # ${p.role}`);
    }
    lines.push("");
  }
  if (sensorEntries.length > 0) {
    ...
    for (const [port, p] of sensorEntries) {
      const varName = toVarName(p.role);
      const portConst = `PORT_${port}`;
      if (p.part === "ultrasonic-sensor") lines.push(`${varName} = Ultrasonic(brick, ${portConst})  # ${p.role}`);
      else if (p.part === "touch-sensor") lines.push(`${varName} = Touch(brick, ${portConst})  # ${p.role}`);
      else if (p.part === "light-sensor") lines.push(`${varName} = Light(brick, ${portConst})  # ${p.role}`);
      else if (p.part === "color-sensor") lines.push(`${varName} = Color20(brick, ${portConst})  # ${p.role}`);
    }
    lines.push("");
  }
  lines.push("# Program");
  if (intent === "beep_hello") {
    lines.push(`brick.play_tone_and_wait(440, 500)  # Beep hello`);
  } else if (intent === "safe_probe") {
    for (const [port, p] of motorEntries) {
      const varName = toVarName(p.role);
      lines.push(`print("Testing ${p.role} (Port ${port})")`);
      lines.push(`${varName}.run(SAFE_POWER)`);
      lines.push(`time.sleep(1)`);
      lines.push(`${varName}.run(-SAFE_POWER)`);
      lines.push(`time.sleep(1)`);
      lines.push(`${varName}.idle()`);
      lines.push(`time.sleep(WAIT_SECS)`);
    }
    lines.push(`brick.play_tone_and_wait(700, 300)`);
    lines.push(`print("Probe complete")`);
  } else if (intent === "drive_forward") {
    const drives = motorEntries.filter(([, p]) => /drive|wheel|track/.test(p.role));
    if (drives.length >= 2) {
      lines.push(`# Drive forward 1 second`);
      for (const [, p] of drives) lines.push(`${toVarName(p.role)}.run(SAFE_POWER)`);
      lines.push(`time.sleep(1)`);
      for (const [, p] of drives) lines.push(`${toVarName(p.role)}.idle()`);
    } else if (drives.length === 1) {
      const [, p] = drives[0];
      lines.push(`${toVarName(p.role)}.run(SAFE_POWER)`);
      lines.push(`time.sleep(1)`);
      lines.push(`${toVarName(p.role)}.idle()`);
    } else {
      lines.push(`# No drive motors found for ${profile.name}`);
    }
  } else if (intent === "wave") {
    ...
    if (arms.length > 0) {
      ...
      lines.push(`brick.play_tone_and_wait(440, 200)  # No arm motor found`);
    }
  } else if (intent === "custom" && customCode) {
    lines.push(String(customCode));
  } else {
    lines.push(`brick.play_tone_and_wait(440, 300)`);
  }
  return lines.join("\n") + "\n";
}
...
function generateRcxNqcCode(profile, intent, customCode) {
  ...
  if (motorEntries.length > 0) {
    lines.push("// Motor port definitions");
    for (const [port, p] of motorEntries) {
      lines.push(`#define ${toVarName(p.role).toUpperCase().padEnd(20)} OUT_${port}  // ${p.role}`);
    }
    lines.push("");
  }
  if (sensorEntries.length > 0) {
    lines.push("// Sensor port definitions");
    for (const [port, p] of sensorEntries) {
      lines.push(`#define ${toVarName(p.role).toUpperCase().padEnd(20)} SENSOR_${port}  // ${p.role}`);
    }
    lines.push("");
  }
  lines.push("#define SAFE_POWER  60  // 0-100");
  lines.push("#define WAIT_10THS  10  // 10ths of a second per movement");
  lines.push("");
  lines.push("task main()");
  lines.push("{");
  if (sensorEntries.length > 0) {
    lines.push("    // Sensor type setup");
    for (const [port, p] of sensorEntries) {
      if (p.part === "touch-sensor") lines.push(`    SetSensor(SENSOR_${port}, SENSOR_TOUCH);`);
      else if (p.part === "light-sensor") lines.push(`    SetSensor(SENSOR_${port}, SENSOR_LIGHT);`);
    }
    lines.push("");
  }
  if (intent === "beep_hello") {
    lines.push(`    PlayTone(440, 3);  // Beep hello`);
    lines.push(`    Wait(30);`);
  } else if (intent === "safe_probe") {
    for (const [port, p] of motorEntries) {
      lines.push(`    // Testing ${p.role} (OUT_${port})`);
      lines.push(`    SetOutput(OUT_${port}, OUT_ON);`);
      lines.push(`    SetPower(OUT_${port}, SAFE_POWER);`);
      lines.push(`    Wait(WAIT_10THS);`);
      lines.push(`    SetOutput(OUT_${port}, OUT_OFF);`);
      lines.push(`    Wait(5);`);
      lines.push(`    SetDirection(OUT_${port}, OUT_REV);`);
      lines.push(`    SetOutput(OUT_${port}, OUT_ON);`);
      lines.push(`    Wait(WAIT_10THS);`);
      lines.push(`    SetOutput(OUT_${port}, OUT_OFF);`);
      lines.push(`    SetDirection(OUT_${port}, OUT_FWD);`);
      lines.push(`    Wait(5);`);
    }
    lines.push(`    PlayTone(700, 3);`);
    lines.push(`    Wait(30);`);
  } else if (intent === "drive_forward") {
    const drives = motorEntries.filter(([, p]) => /drive|wheel|track/.test(p.role));
    if (drives.length >= 2) {
      const mask = drives.map(([port]) => `OUT_${port}`).join(" | ");
      lines.push(`    // Drive forward (~2 seconds)`);
      lines.push(`    SetOutput(${mask}, OUT_ON);`);
      lines.push(`    SetPower(${mask}, SAFE_POWER);`);
      lines.push(`    Wait(20);`);
      lines.push(`    SetOutput(${mask}, OUT_OFF);`);
    } else if (drives.length === 1) {
      const [port] = drives[0];
      lines.push(`    SetOutput(OUT_${port}, OUT_ON);`);
      lines.push(`    SetPower(OUT_${port}, SAFE_POWER);`);
    ...
    if (arms.length > 0) {
      ...
      lines.push(`    PlayTone(440, 3);  // No arm motor found for wave`);
      lines.push(`    Wait(30);`);
    }
  } else if (intent === "custom" && customCode) {
    const customLines = String(customCode).split("\n").map(l => `    ${l}`).join("\n");
    lines.push(customLines);
  } else {
    lines.push(`    PlayTone(440, 3);`);
    lines.push(`    Wait(30);`);
  }
  lines.push("}");
  return lines.join("\n") + "\n";
...
function generateWeDo2Code(profile, intent, customCode) {
  ...
  const lines = [
    ...
    `# Generated by lego-mindstorms-mcp`,
    `# NOTE: WeDo 2.0 App available until July 31 2026 — consider Pybricks CityHub for ongoing use.`,
    ``,
    `import hub`,
    `import time`,
    ``,
  ...
  if (motorEntries.length > 0) {
    lines.push("# Motor bindings — Port A = connector 1, Port B = connector 2");
    for (const [port, p] of motorEntries)
      lines.push(`${toVarName(p.role)} = hub.port.${port}.motor  # ${p.role}`);
    lines.push("");
  }
  if (sensorEntries.length > 0) {
    lines.push("# Sensor bindings");
    for (const [port, p] of sensorEntries)
      lines.push(`${toVarName(p.role)} = hub.port.${port}.device  # ${p.role}`);
    lines.push("");
  }
  lines.push("# Program");
  if (intent === "beep_hello") {
    lines.push(`hub.sound.beep()`, `hub.led(6)  # green`, `time.sleep(1)`, `hub.led(0)  # off`);
  } else if (intent === "safe_probe") {
    lines.push(`hub.led(7)  # yellow`);
    for (const [port, p] of motorEntries) {
    ...
    lines.push(`hub.led(6)  # green`, `hub.sound.beep()`, `print("Probe complete")`);
  } else if (intent === "drive_forward") {
    const drives = motorEntries.filter(([, p]) => /drive|wheel|tilt/.test(p.role));
    const targets = drives.length ? drives : motorEntries.slice(0, 1);
    if (targets.length) {
      lines.push(`# Drive forward 1 second`);
      targets.forEach(([, p]) => lines.push(`${toVarName(p.role)}.start(speed=SAFE_SPEED)`));
      lines.push(`time.sleep(1)`);
      targets.forEach(([, p]) => lines.push(`${toVarName(p.role)}.stop()`));
    } else {
      lines.push(`# No motor found — connect WeDo 2.0 Medium Motor to Port A`);
    }
  } else if (intent === "wave") {
    ...
    if (m) {
      ...
      lines.push(`for _ in range(3):`,
        ...
        `    ${v}.run_for_seconds(speed=-SAFE_SPEED, seconds=0.3)`,
        `    time.sleep(0.1)`);
    } else { lines.push(`hub.sound.beep()  # No motor found`); }
  } else if (intent === "custom" && customCode) {
    lines.push(String(customCode));
  } else {
    lines.push(`hub.sound.beep()`);
  }
  return lines.join("\n") + "\n";
}
...
function generatePybricksCityCode(profile, intent, customCode) {
  ...
  const lines = [
    ...
    `# Hub: LPF2 Smart Hub 2 I/O  |  Generated by lego-mindstorms-mcp`,
    ``,
  ...
  if (motorEntries.length > 0) {
    ...
    for (const [port, p] of motorEntries) {
      const dir = p.positiveDirection === "counterclockwise"
        ? `, positive_direction=Direction.COUNTERCLOCKWISE` : "";
      lines.push(`${toVarName(p.role)} = Motor(Port.${port}${dir})  # ${p.role}`);
    }
    lines.push("");
  }
  if (sensorEntries.length > 0) {
    ...
    for (const [port, p] of sensorEntries) {
      const v = toVarName(p.role);
      if (p.part === "wedo2-motion-sensor") lines.push(`${v} = UltrasonicSensor(Port.${port})  # ${p.role}`);
      else if (p.part === "color-sensor")   lines.push(`${v} = ColorSensor(Port.${port})  # ${p.role}`);
    }
    lines.push("");
  }
  lines.push("# Program");
  if (intent === "beep_hello") {
    lines.push(`hub.light.on(Color.GREEN)`, `wait(1000)`, `hub.light.off()`);
  } else if (intent === "safe_probe") {
    lines.push(`hub.light.on(Color.YELLOW)`);
    for (const [port, p] of motorEntries) {
      const v = toVarName(p.role);
    ...
    lines.push(`hub.light.on(Color.GREEN)`, `print("Probe complete")`);
  } else if (intent === "drive_forward") {
    const drives = motorEntries.filter(([, p]) => /drive|wheel|tilt/.test(p.role));
    const targets = drives.length ? drives : motorEntries.slice(0, 1);
    if (targets.length >= 2) {
      lines.push(`# Drive forward 1 second`);
      targets.forEach(([, p]) => lines.push(`${toVarName(p.role)}.run(SAFE_SPEED)`));
      lines.push(`wait(1000)`);
      targets.forEach(([, p]) => lines.push(`${toVarName(p.role)}.stop()`));
    } else if (targets.length === 1) {
      lines.push(`${toVarName(targets[0][1].role)}.run_angle(SAFE_SPEED, SAFE_ANGLE)`);
    } else {
      lines.push(`# No motor found — connect WeDo 2.0 Medium Motor to Port A`);
    }
  } else if (intent === "wave") {
    ...
    if (m) {
      ...
      lines.push(`for _ in range(3):`,
        ...
        `    ${v}.run_angle(SAFE_SPEED, -90)`, `    wait(200)`);
    } else { lines.push(`hub.light.on(Color.GREEN)  # No motor found`); }
  } else if (intent === "custom" && customCode) {
    lines.push(String(customCode));
  } else {
    lines.push(`hub.light.on(Color.GREEN)`, `wait(500)`, `hub.light.off()`);
  }
  return lines.join("\n") + "\n";
}
...
async function handleAction(action, params = {}) {
  ...
  if (action === "code_generate") {
    ...
    const validTargets = new Set([
      "pybricks-python", "lego-stock-python",
      "pybricks-ev3", "ev3dev-python",
      "nxt-python",
      "rcx-nqc",
      "wedo2-micropython", "pybricks-city"
    ]);
